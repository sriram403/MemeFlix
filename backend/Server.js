...
const baseMemeSelectFields = (userId) => ` m.id, m.title, m.description, m.filename, m.type, m.filepath, m.upvotes, m.downvotes, m.uploaded_at, GROUP_CONCAT(DISTINCT t.name) as tags, ${userId ? `MAX(CASE WHEN vh.user_id = ? THEN 1 ELSE 0 END) as is_viewed` : '0 as is_viewed'}, ${userId ? `COALESCE(uv.vote_type, 0) as user_vote_status` : '0 as user_vote_status'} `;
const baseMemeJoins = (userId) => ` FROM memes m LEFT JOIN meme_tags mt ON m.id = mt.meme_id LEFT JOIN tags t ON mt.tag_id = t.tag_id ${userId ? `LEFT JOIN viewing_history vh ON m.id = vh.meme_id AND vh.user_id = ?` : ''} ${userId ? `LEFT JOIN user_votes uv ON m.id = uv.meme_id AND uv.user_id = ?` : ''} `;
// --- REVISED Helper to add user ID parameter(s) if needed ---
const addUserIdParamIfNeeded = (baseParams, userId) => {
    let userParams = [];
    if (userId) {
        // Params for JOINs (viewing_history, user_votes) and SELECT (is_viewed)
        userParams = [userId, userId, userId]; // Add three userIds if user exists
    }
    // Combine user params FIRST, then base params (like limit, offset)
...
app.get('/api/memes', getUserIfAuthenticated, async (req, res) => {
  ...
  const whereSql = userId ? 'WHERE m.id IS NOT NULL' : '';
  ...
  try {
    ...
    console.error("SQL:", sqlGetData);
    console.error("Params:", params);
    res.status(500).json({ error: 'Failed to retrieve memes.' });
  }
});

// /api/memes/random (Unchanged, but ensure param logic is right)
app.get('/api/memes/random', getUserIfAuthenticated, async (req, res) => { const userId = req.user ? req.user.id : null; const sql = ` SELECT ${baseMemeSelectFields(userId)} ${baseMemeJoins(userId)} WHERE m.id IS NOT NULL GROUP BY m.id ORDER BY RANDOM() LIMIT 1 `; const params = addUserIdParamIfNeeded([], userId); try { const randomMeme = await getDb(sql, params); if (!randomMeme) { return res.status(404).json({ error: 'No memes found.' }); } res.status(200).json({ meme: randomMeme }); } catch (err) { console.error("DB fetch random meme error:", err.message); res.status(500).json({ error: 'Failed to retrieve a random meme.' }); } });
// /api/memes/search (Unchanged, but ensure param logic is right)
app.get('/api/memes/search', getUserIfAuthenticated, async (req, res) => { const query = req.query.q || ''; const filterType = req.query.type || ''; const sortBy = req.query.sort || 'newest'; const filterTag = req.query.tag || ''; const page = parseInt(req.query.page || '1', 10); const limit = parseInt(req.query.limit || '12', 10); if (isNaN(page) || page < 1 || isNaN(limit) || limit < 1) { return res.status(400).json({ error: 'Invalid page or limit parameter.' }); } const offset = (page - 1) * limit; const userId = req.user ? req.user.id : null; let whereClauses = []; let filterParams = []; let countParams = []; let joins = baseMemeJoins(userId); let countJoins = baseMemeJoins(null); if (query) { const condition = `(lower(m.title) LIKE ? OR lower(m.description) LIKE ? OR lower(m.filename) LIKE ?)`; whereClauses.push(condition); const searchTerm = `%${query.toLowerCase()}%`; filterParams.push(searchTerm, searchTerm, searchTerm); countParams.push(searchTerm, searchTerm, searchTerm); } const validTypes = ['image', 'gif', 'video']; if (filterType && validTypes.includes(filterType.toLowerCase())) { whereClauses.push(`lower(m.type) = ?`); filterParams.push(filterType.toLowerCase()); countParams.push(filterType.toLowerCase()); } if (filterTag) { joins = ` FROM memes m INNER JOIN meme_tags mt_filter ON m.id = mt_filter.meme_id INNER JOIN tags t_filter ON mt_filter.tag_id = t_filter.tag_id LEFT JOIN meme_tags mt ON m.id = mt.meme_id LEFT JOIN tags t ON mt.tag_id = t.tag_id ${userId ? `LEFT JOIN viewing_history vh ON m.id = vh.meme_id AND vh.user_id = ?` : ''} ${userId ? `LEFT JOIN user_votes uv ON m.id = uv.meme_id AND uv.user_id = ?` : ''} `; countJoins = ` FROM memes m INNER JOIN meme_tags mt_filter ON m.id = mt_filter.meme_id INNER JOIN tags t_filter ON mt_filter.tag_id = t_filter.tag_id `; whereClauses.push(`lower(t_filter.name) = lower(?)`); filterParams.push(filterTag); countParams.push(filterTag); } const baseCondition = 'm.id IS NOT NULL'; const whereSql = whereClauses.length > 0 ? `WHERE ${whereClauses.join(' AND ')}` : `WHERE ${baseCondition}`; let orderBySql = 'ORDER BY '; switch (sortBy.toLowerCase()) { case 'oldest': orderBySql += 'm.uploaded_at ASC'; break; case 'score': orderBySql += '(m.upvotes - m.downvotes) DESC, m.uploaded_at DESC'; break; case 'newest': default: orderBySql += 'm.uploaded_at DESC'; break; } const sqlGetData = ` SELECT ${baseMemeSelectFields(userId)} ${joins} ${whereSql} GROUP BY m.id ${orderBySql} LIMIT ? OFFSET ? `; const dataParams = addUserIdParamIfNeeded([...filterParams, limit, offset], userId); const sqlGetCount = ` SELECT COUNT(DISTINCT m.id) as totalMemes ${countJoins} ${whereSql} `; try { const [totalRow, memesForPage] = await Promise.all([ getDb(sqlGetCount, countParams), allDb(sqlGetData, dataParams) ]); const totalMemes = totalRow?.totalMemes || 0; const totalPages = Math.ceil(totalMemes / limit); res.status(200).json({ memes: memesForPage || [], pagination: { currentPage: page, totalPages: totalPages, totalMemes: totalMemes, limit: limit } }); } catch (err) { console.error("DB search error:", err.message); console.error("SQL Count:", sqlGetCount); console.error("Count Params:", countParams); console.error("SQL Data:", sqlGetData); console.error("Data Params:", dataParams); return res.status(500).json({ error: 'Search failed due to database error.' }); } });
...
// Favorites Routes (Updated to include user data for is_viewed/vote_status)
...
// History Routes (Updated to include user data for is_viewed/vote_status)
