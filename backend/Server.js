...
// Middleware to potentially get user ID without requiring auth for all routes
const getUserIfAuthenticated = (req, res, next) => {
    ...
    jwt.verify(token, JWT_SECRET, (err, user) => {
        if (err) {
            ...
            req.user = user; // Attach user info if token is valid
        }
        next();
    });
};
...
// Require authentication specifically where needed
const authenticateToken = (req, res, next) => {
  if (!req.user) { // Check if user was attached by previous middleware
    return res.sendStatus(401); // If not, send Unauthorized
  }
  next(); // If user exists, proceed
};
...
// --- Base Meme Select Query with Tags & **Conditional Viewed Status** ---
// Select fields now include a conditional is_viewed flag
...
`;
// Joins now conditionally include viewing_history if userId is provided
...
// Helper to add user ID parameter if needed
const addUserIdParamIfNeeded = (baseParams, userId) => {
    const params = [...baseParams];
    if (userId) {
        params.unshift(userId); // For the is_viewed CASE statement
        params.unshift(userId); // For the viewing_history JOIN condition
    }
    return params;
};
const addUserIdParamForSelectOnly = (baseParams, userId) => {
    const params = [...baseParams];
    if (userId) {
        params.unshift(userId); // For the is_viewed CASE statement ONLY
    }
    return params;
}
...
// --- UPDATED Meme Routes ---
// Use getUserIfAuthenticated middleware for routes that might show viewed status
app.get('/api/memes', getUserIfAuthenticated, async (req, res) => {
  ...
  try {
    const [totalRow, memesForPage] = await Promise.all([
      getDb(sqlGetCount), // Count doesn't need user ID
...
app.get('/api/memes/random', getUserIfAuthenticated, async (req, res) => {
    ...
    const sql = `
        ...
        LIMIT 1
    `;
     const params = addUserIdParamIfNeeded([], userId);
    try {
...
app.get('/api/memes/search', getUserIfAuthenticated, async (req, res) => {
    ...
    const page = parseInt(req.query.page || '1', 10);
    const limit = parseInt(req.query.limit || '12', 10);
    if (isNaN(page) || page < 1 || isNaN(limit) || limit < 1) { return res.status(400).json({ error: 'Invalid page or limit parameter.' }); }
    const offset = (page - 1) * limit;
    const userId = req.user ? req.user.id : null;

    let whereClauses = [];
    let filterParams = []; // Params for WHERE clause only
    let joins = baseMemeJoins(userId); // Start with base joins (conditionally includes history join)
    let countJoins = baseMemeJoins(null); // Count joins don't need history table

    if (query) { const condition = `(lower(m.title) LIKE ? OR lower(m.description) LIKE ? OR lower(m.filename) LIKE ?)`; whereClauses.push(condition); const searchTerm = `%${query.toLowerCase()}%`; filterParams.push(searchTerm, searchTerm, searchTerm); }
    const validTypes = ['image', 'gif', 'video']; if (filterType && validTypes.includes(filterType.toLowerCase())) { whereClauses.push(`lower(m.type) = ?`); filterParams.push(filterType.toLowerCase()); }
    if (filterTag) {
        // Inner join for filtering, outer joins for getting all tags/history
        joins = ` FROM memes m INNER JOIN meme_tags mt_filter ON m.id = mt_filter.meme_id INNER JOIN tags t_filter ON mt_filter.tag_id = t_filter.tag_id LEFT JOIN meme_tags mt ON m.id = mt.meme_id LEFT JOIN tags t ON mt.tag_id = t.tag_id ${userId ? `LEFT JOIN viewing_history vh ON m.id = vh.meme_id AND vh.user_id = ?` : ''} `;
        countJoins = ` FROM memes m INNER JOIN meme_tags mt_filter ON m.id = mt_filter.meme_id INNER JOIN tags t_filter ON mt_filter.tag_id = t_filter.tag_id `; // Joins for count
        whereClauses.push(`lower(t_filter.name) = lower(?)`);
        filterParams.push(filterTag);
    }

    const baseCondition = 'm.id IS NOT NULL';
    const whereSql = whereClauses.length > 0 ? `WHERE ${whereClauses.join(' AND ')}` : `WHERE ${baseCondition}`;
    ...
    const dataParams = addUserIdParamIfNeeded([...filterParams, limit, offset], userId); // Add user ID params AFTER filter params

    const sqlGetCount = ` SELECT COUNT(DISTINCT m.id) as totalMemes ${countJoins} ${whereSql} `;
    const countParams = [...filterParams]; // Count uses only filter params

    try {
        const [totalRow, memesForPage] = await Promise.all([ getDb(sqlGetCount, countParams), allDb(sqlGetData, dataParams) ]);
        const totalMemes = totalRow?.totalMemes || 0;
        const totalPages = Math.ceil(totalMemes / limit);
...
// Favorites Routes (Needs getUserIfAuthenticated for consistency if showing viewed status)
