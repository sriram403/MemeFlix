...
function HomePage() {
    ...
    const handleVote = useCallback(async (memeId, voteType) => {
        ...
        // Store original states for potential revert ONLY IF NEEDED (context handles state now)
        // We still need to update the visual counts immediately though
        const originalTagMemes = {...tagMemes};
        const originalSelectedMeme = selectedMeme ? {...selectedMeme} : null;

        // --- Optimistic UI Update for COUNTS (vote status handled by context) ---
        const voteChange = voteType === 'upvote' ? 1 : -1;
        const currentVoteStatus = getUserVoteStatus(memeId); // Get current vote (-1, 0, 1)
        ...
        // Update the specific tag row state in 'tagMemes'
        let updated = false;
        ...
        if (updated) { setTagMemes(newTagMemes); }

        // Update the 'selectedMeme' state if the voted meme is open in the modal
        ...
        // --- End Optimistic UI Update for COUNTS ---

        // --- Call Context Function for API and State ---
        const result = await submitVote(memeId, voteType);

        if (!result.success) {
            setError(result.error || `Failed to record ${voteType}.`); // Show error message
    ...
    }, [isAuthenticated, tagMemes, selectedMeme, submitVote, getUserVoteStatus]); // Added submitVote, getUserVoteStatus
        ...
        <div className="home-page">
            ...
            {!loadingTags && popularTags.map(tagInfo => (
                <MemeRow
                    ...
                    // Pass down getUserVoteStatus if needed for visual vote indication on card
