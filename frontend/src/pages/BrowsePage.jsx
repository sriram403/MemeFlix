...
function BrowsePage() {
    ...
    // --- Define Modal Handlers BEFORE the effect that uses them ---
    const openModal = useCallback((meme) => {
        setSelectedMeme(meme);
        setIsModalOpen(true);
        recordView(meme.id);
    }, [recordView]); // recordView is stable from context
            ...
            if (openIdParam) {
                ...
                if (memeToOpen) {
                    ...
                    if (!isModalOpen || selectedMeme?.id !== memeIdToOpen) {
                        openModal(memeToOpen); // Now openModal is defined
                    }
                    setSearchParams(prev => {
                        prev.delete('openMemeId');
                        return prev;
    ...
    // Now the dependencies are correct
    }, [loading, memes, searchParams, setSearchParams, openModal, isModalOpen, selectedMeme?.id]);
    ...
    return (
        <div className="browse-page">
            ...
            {!(currentQuery || currentType || currentTag) && pagination.totalPages > 1 && !isGridLoading && (
                 <PaginationControls
