...
function MemeDetailModal({ meme, onClose, onVote, onFavoriteToggle }) {
    // *** REMOVE getUserVoteStatus from context destructuring ***
    ...
    // --- Get current vote status directly from the meme prop ---
    const currentUserVote = meme.user_vote_status || 0; // Use API flag, default to 0
    ...
    // Event Handlers (Unchanged) - onVote is passed from parent now
          ...
          <div className="modal-content" onClick={handleContentClick} role="dialog" aria-modal="true" aria-labelledby={modalTitleId} aria-describedby={meme.description ? modalDescriptionId : undefined} tabIndex="-1" >
            ...
            <div className="modal-info">
              ...
              <div className="modal-actions" role="toolbar" aria-label="Meme Actions">
                 {/* Active class uses currentUserVote derived from meme prop */}
                 <button className={`vote-button upvote ${currentUserVote === 1 ? 'vote-button--active' : ''}`} onClick={handleUpvote} aria-pressed={currentUserVote === 1} aria-label={`Upvote meme. Current upvotes: ${meme.upvotes ?? 0}`}> 😂 <span className="vote-count" aria-hidden="true">{meme.upvotes ?? 0}</span> </button>
                 <span className="score" aria-label={`Current score ${score}`} role="status">Score: {score}</span>
                 <button className={`vote-button downvote ${currentUserVote === -1 ? 'vote-button--active' : ''}`} onClick={handleDownvote} aria-pressed={currentUserVote === -1} aria-label={`Downvote meme. Current downvotes: ${meme.downvotes ?? 0}`}> 😑 <span className="vote-count" aria-hidden="true">{meme.downvotes ?? 0}</span> </button>
