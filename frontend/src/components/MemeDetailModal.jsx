...
function MemeDetailModal({ meme, onClose, onVote, onFavoriteToggle }) {
  ...
  // --- Reset media ready state ONLY when the actual meme ID changes ---
  useEffect(() => {
      // console.log('[Modal Debug] Meme ID changed. Resetting isMediaReady.');
      setIsMediaReady(false);
      const timer = setTimeout(() => {
        // console.log('[Modal Debug] Setting isMediaReady to true after delay.');
        setIsMediaReady(true);
      }, 50); // Shorter delay might be fine now
      return () => clearTimeout(timer);
  // *** CHANGE DEPENDENCY FROM [meme] to [meme?.id] ***
  }, [meme?.id]);
  ...
  return (
    <div className="modal-overlay" onClick={onClose} role="presentation">
      ...
      <div
        ...
        aria-describedby={meme.description ? modalDescriptionId : undefined}
        tabIndex="-1"
      >
        ...
        <div className="modal-info">
          ...
          <div className="modal-actions" role="toolbar" aria-label="Meme Actions">
             <button className="vote-button upvote" onClick={handleUpvote} aria-label={`Upvote meme. Current upvotes: ${meme.upvotes ?? 0}`}> 😂 <span className="vote-count" aria-hidden="true">{meme.upvotes ?? 0}</span> </button>
             <span className="score" aria-label={`Current score ${score}`} role="status">Score: {score}</span>
             <button className="vote-button downvote" onClick={handleDownvote} aria-label={`Downvote meme. Current downvotes: ${meme.downvotes ?? 0}`}> 😑 <span className="vote-count" aria-hidden="true">{meme.downvotes ?? 0}</span> </button>
